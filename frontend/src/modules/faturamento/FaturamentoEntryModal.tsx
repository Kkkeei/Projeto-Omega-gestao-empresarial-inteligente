...
type CompetenciaExistente = MesFaturamento;
...
function quarterRange(year: number, month: number) {
  ...
  return {
    start: monthKey(year, start),
    end: monthKey(year, start + 2),
  };
}
...
export function FaturamentoEntryModal({
  ...
  const [dataFaturamento, setDataFaturamento] = useState(
    dateForMonth(defaultYear, defaultMonth),
  );
  ...
  useEffect(() => {
    ...
    const initialDate =
      initial?.data_faturamento ||
      dateForMonth(
        initial?.competencia_ano ?? defaultYear,
        initial?.competencia_mes ?? defaultMonth,
      );
    ...
    setValor(
      initial?.valor == null
        ? ""
        : String(initial.valor.toFixed(2)).replace(".", ","),
    );
    ...
    open,
    initial?.id,
    initial?.competencia_ano,
    initial?.competencia_mes,
    initial?.data_faturamento,
  ...
  useEffect(() => {
    ...
    const ranges =
      periodicidade === "Anual"
        ? {
            start: monthKey(defaultYear, 1),
            end: monthKey(defaultYear, 12),
          }
    ...
    const generated = clampMonthRange(ranges.start, ranges.end).map((item) => {
      const existing = existingFaturamentos.find(
        (x) =>
          x.competencia_ano === item.year && x.competencia_mes === item.month,
      );
      ...
      return {
        ...
        valor:
          existing?.valor == null
            ? ""
            : String(existing.valor.toFixed(2)).replace(".", ","),
  ...
  const intervalo = useMemo(
    () => clampMonthRange(periodoInicio, periodoFim),
    [periodoInicio, periodoFim],
  );

  const dataCompetencia = parseDateYearMonth(dataFaturamento);

  const existingForDate = existingFaturamentos.find(
    (x) =>
      x.competencia_ano === dataCompetencia.year &&
      x.competencia_mes === dataCompetencia.month,
  );
  ...
  function alterarPeriodicidade(next: Periodicidade) {
    ...
    if (next === "Anual") {
      ...
      setPeriodoInicio(range.start);
      setPeriodoFim(range.end);
    ...
    try {
      if (periodicidade === "Mensal") {
        if (
          ...
          dataCompetencia.month < 1 ||
          dataCompetencia.month > 12
        ) {
        ...
        if (
          initial?.id &&
          (initial.competencia_ano !== dataCompetencia.year ||
            initial.competencia_mes !== dataCompetencia.month)
        ) {
          throw new Error(
            `Para editar este lançamento, a data deve continuar em ${MESES[initial.competencia_mes]}/${initial.competencia_ano}. Para trocar a competência, informe um novo faturamento.`,
          );
        }

        const dateObject = new Date(dataFaturamento + "T12:00:00");

        const maxDay = lastDayOfMonth(
          dataCompetencia.year,
          dataCompetencia.month,
        );

        if (dateObject.getDate() > maxDay) {
          throw new Error(
            "A data selecionada não é válida para esta competência.",
          );
        }
        ...
        const preenchidos = itens
          ...
          .map((item) => {
            ...
            if (!Number.isFinite(n) || n < 0) {
              throw new Error(
                `Valor inválido em ${MESES[item.month]}/${item.year}.`,
              );
            }
        ...
        if (!preenchidos.length) {
          throw new Error("Informe ao menos uma competência para salvar.");
        }
  ...
  return (
    <div
      className="fat-modal-backdrop"
      onMouseDown={() => !saving && onClose()}
    >
      <div
        className="fat-modal fat-entry-modal"
        onMouseDown={(e) => e.stopPropagation()}
      >
        <div className="fat-modal-header">
          <div>
            <span className="fat-modal-icon">
              <CalendarDays size={17} />
            </span>

            <div>
              <strong>
                {initial?.id ? "Editar faturamento" : "Informar faturamento"}
              </strong>
        ...
        <div className="fat-entry-type-tabs">
          {(
            ...
            <button
              type="button"
              key={item}
              className={periodicidade === item ? "active" : ""}
              onClick={() => alterarPeriodicidade(item)}
              disabled={saving}
            >
              {item}
            </button>
        ...
        {periodicidade === "Mensal" ? (
          <div className="fat-modal-body">
            <div className="fat-entry-two-fields">
              ...
              <div className="fat-form-highlight compact">
                <span>Competência</span>

                <strong>
                  {dataCompetencia.year && MESES[dataCompetencia.month]
                    ? `${MESES[dataCompetencia.month]}/${dataCompetencia.year}`
                    : "—"}
                </strong>
              </div>
            </div>

            {existingForDate && !initial?.id && (
              <div className="fat-entry-info">
                Já existe faturamento informado nesta competência. Ao salvar, a
                plataforma atualizará o lançamento existente.
              </div>
            )}
            ...
            <label>
              Observação <span className="fat-label-help">Opcional</span>
              <textarea
                value={observacao}
                onChange={(e) => setObservacao(e.target.value)}
                placeholder="Observação sobre este faturamento..."
              />
            </label>
          </div>
        ) : (
          <div className="fat-modal-body">
            <div className="fat-form-highlight">
              <span>Período do lançamento</span>

              <strong>
                {intervalo.length
                  ? `${MESES[intervalo[0].month]}/${intervalo[0].year} a ${MESES[intervalo[intervalo.length - 1].month]}/${intervalo[intervalo.length - 1].year}`
                  : "Selecione um período válido"}
              </strong>

              <small>
                O valor é registrado competência por competência para preservar
                os dados mensais da plataforma.
              </small>
            </div>
            ...
            {periodicidade === "Anual" && (
              <label>
                Ano-base *
                <input type="number" value={defaultYear} readOnly />
              </label>
            )}

            {periodicidade !== "Anual" && (
              <div className="fat-period-picker">
                <label>
                  De *
                  <input
                    type="month"
                    value={periodoInicio}
                    onChange={(e) => alterarPeriodo("inicio", e.target.value)}
                  />
                </label>
                ...
                <label>
                  ...
                  <input
                    type="month"
                    value={periodoFim}
                    onChange={(e) => alterarPeriodo("fim", e.target.value)}
                  />
                </label>
              </div>
            )}
            ...
            <div className="fat-batch-table-wrap">
              <table className="fat-batch-table">
                <thead>
                  <tr>
                    <th>Competência</th>
