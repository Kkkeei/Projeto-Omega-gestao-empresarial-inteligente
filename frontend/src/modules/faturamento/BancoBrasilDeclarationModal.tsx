...
export function BancoBrasilDeclarationModal({
  ...
  const soma =
    Number(config.percentual_a_vista || 0) +
    Number(config.percentual_a_prazo || 0);

  function atualizar(
    campo: "percentual_a_vista" | "percentual_a_prazo",
    valor: string,
  ) {
  ...
  async function abrirPdf() {
    ...
    try {
      ...
      setError(
        e instanceof Error
          ? e.message
          : "Não foi possível abrir o PDF da declaração.",
      );
    }
  }
  ...
  async function gerar() {
    ...
    try {
      ...
      setGerada({
        id: declaracao.id,
        nome_arquivo: declaracao.nome_arquivo,
      });
      ...
      setError(
        e instanceof Error
          ? e.message
          : "Não foi possível gerar a declaração do Banco do Brasil.",
      );
    }
  }
  ...
  return (
    <div
      ...
      onMouseDown={() => !gerando && onClose()}
    >
      <div
        className="fat-modal fat-bb-modal"
      ...
      >
        ...
        {!gerando && !gerada && (
          <div className="fat-modal-body">
            <div className="fat-form-highlight">
              <span>Percentuais da declaração</span>
            ...
            <div className="fat-entry-two-fields">
              <label>
                ...
                <input
                  ...
                  onChange={(event) =>
                    atualizar("percentual_a_vista", event.target.value)
                  }
                />
              </label>
              ...
              <label>
                ...
                <input
                  ...
                  onChange={(event) =>
                    atualizar("percentual_a_prazo", event.target.value)
                  }
                />
              </label>
            </div>
            ...
            <div
              className={`fat-form-highlight compact ${
                Math.abs(soma - 100) > 0.01 ? "invalid" : ""
              }`}
            >
              <span>Total da divisão</span>

              <strong>{soma.toFixed(2).replace(".", ",")}%</strong>
            </div>

            {error && (
              <div className="fat-entry-error form-error">
                <X size={14} />
                {error}
              </div>
            )}
        ...
        {gerando && (
          <div className="fat-bb-progress">
            ...
            <span>
              O ÔMEGA está preenchendo o formulário oficial e preparando o PDF.
            </span>

            <small>
              Isso pode levar alguns segundos porque depende do site do Banco do
              Brasil.
            </small>
          </div>
        )}
