...
export function DocumentacaoPage(){
 ...
 return <div className="page documentacao-page">
  ...
  {loading?<Loading text="Carregando empresas..."/>:err?<ErrorState message={err} onRetry={load}/>:<div className="doc-company-list">{filtered.map(e=><article className="doc-company-row" key={e.id}><div className="doc-company-main"><span className="empresa-icon"><Building2 size={17}/></span><div className="doc-company-name"><strong>{e.razao_social}</strong><small>{e.nome_fantasia||'Sem nome fantasia'}</small></div></div><div className="doc-company-data"><span><b>CNPJ</b>{e.cnpj}</span><span><b>Regime</b><em className={regimeClass(e.regime_tributario)}>{e.regime_tributario||'Não informado'}</em></span><span><b>Status</b><em className={`status-pill ${e.ativo?'ok':'off'}`}><i/>{e.ativo?'Ativa':'Inativa'}</em></span><span className="doc-count"><b>Documentos</b><strong><Folder size={14}/>{e.documentos_count}</strong></span></div><Link className="button secondary doc-open" to={`/documentacao/empresa/${e.id}`}>Abrir documentação <ArrowRight size={13}/></Link></article>)}{filtered.length===0&&<div className="empty"><Folder size={32}/><strong>Nenhuma empresa encontrada</strong><span>Ajuste os filtros ou a pesquisa.</span></div>}</div>}
 </div>
}
