...
from .repository import criar_declaracao, listar_faturamentos_empresa, listar_faturamentos_periodo, obter_empresa
...
def periodo_12_meses_por_empresa(empresa_id: int) -> tuple[int, int, int, int, bool]:
    ...
    # Procuramos o último lançamento existente até a competência atual, inclusive
    # quando o último registro está em um ano anterior.
    faturamentos = [
        item
        for item in listar_faturamentos_empresa(empresa_id)
        if (int(item["competencia_ano"]), int(item["competencia_mes"])) <= (limite_ano, limite_mes)
    ]
    if faturamentos:
        ultimo = max(
            faturamentos,
            key=lambda item: (int(item["competencia_ano"]), int(item["competencia_mes"])),
        )
        fim_ano = int(ultimo["competencia_ano"])
        fim_mes = int(ultimo["competencia_mes"])
